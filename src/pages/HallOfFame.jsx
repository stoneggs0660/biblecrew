...
export default function HallOfFame(){
  ...
  useEffect(() => {
    ...
    const medalToCrews = {
      gold: ['고급반'],
      silver: ['중급반'],
      bronze: CREW_KEYS.filter(k => String(k).startsWith('초급반')),
    };
          ...
          (['gold','silver','bronze']).forEach((medal) => {
            ...
            crewList.forEach((crewKey) => {
              ...
              Object.entries(usersNode).forEach(([userKey, u]) => {
                ...
                if (ok) successNamesByMedal[medal].push(displayName);
              });
            });
          });
      ...
      {activeYear && selectedYear === activeYear && (
        <div style={{marginBottom:24, padding:16, background:'#fff', borderRadius:8, boxShadow:'0 4px 6px rgba(0,0,0,0.08)'}}>
          ...
          <div style={{fontSize:12, color:'#777', marginTop:8}}>
            ※ 월중에는 "성공/실패"가 확정값이 아니라 현재 상태(달성/미달)입니다.
          </div>
        </div>
      )}
            ...
            <div key={m} style={{marginBottom:12}}>
              ...
              {hasAny && (
                <div style={{marginLeft:10, fontSize:14}}>
                  <div>
                    금메달 🥇 : {gold && gold.length ? gold.join(' · ') : '없음'}
                  </div>
                  <div>
                    은메달 🥈 : {silver && silver.length ? silver.join(' · ') : '없음'}
                  </div>
                  <div>
                    동메달 🥉 : {bronze && bronze.length ? bronze.join(' · ') : '없음'}
                  </div>
                </div>
              )}
            </div>
