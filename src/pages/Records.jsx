...
export default function Records({ user }) {
  ...
  // 해당 월의 최소 장 수(금/은/동 기준)를 개인 기록에 반영한다.
  if (user && user.uid && monthlyHoF) {
    const GOLD_CHAPTERS = 1189;
    const SILVER_CHAPTERS = 929;
    const BRONZE_CHAPTERS = 260;

    const monthNode = monthlyHoF[currentMonth];
    if (monthNode) {
      let target = 0;
      if (monthNode.gold && monthNode.gold[user.uid]) {
        target = GOLD_CHAPTERS;
      } else if (monthNode.silver && monthNode.silver[user.uid]) {
        target = SILVER_CHAPTERS;
      } else if (monthNode.bronze && monthNode.bronze[user.uid]) {
        target = BRONZE_CHAPTERS;
      }

      if (target > 0 && target > monthChapters) {
        const diff = target - monthChapters;
        monthChapters = target;
        yearChapters += diff; // 같은 해의 누적 장 수에도 반영
      }
    }
  }
